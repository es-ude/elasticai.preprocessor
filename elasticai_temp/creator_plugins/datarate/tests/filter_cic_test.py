...
def test_filter_cic(cocotb_test_fixture: CocotbTestFixture, bitwidth: int, dec_rate: int, n_dec: int):
    ...
    cocotb_test_fixture.add_srcs_from_package("datarate", "verilog/filter_cic.v")
    cocotb_test_fixture.run(params={"BITWIDTH": bitwidth, "DEC_RATE": dec_rate, "N_DEC": n_dec}, defines={})
...
def test_filter_cic_build(cocotb_test_fixture: CocotbTestFixture, bitwidth: int, dec_rate: int, n_dec: int):
    artifact_dir = cocotb_test_fixture.get_artifact_dir()
    build_dir = artifact_dir / "verilog"

    load_and_plugin( # nimmt verilog template mit angepassten Daten und fügt sie hinzu
        type="filter_cic",
        id="",
