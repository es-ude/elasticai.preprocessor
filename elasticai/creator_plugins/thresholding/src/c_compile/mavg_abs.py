...
def build_thresholding_mavg_abs(
    gain: float,
    window_size: int,
    bitwidth: int,
    signed: bool,
    ...
    """Generate C files for moving absolute average thresholding.
    Args:
    ...
    params = {
        ...
        "template_name": "thresholding_mavg_abs_template.h",
        "module_id": module_id,
        "device_id": module_id.upper(),
        "data_type": get_embedded_datatype(bitwidth, signed),
    }
    template_c = _generate_thresholding_mavg_abs_template()
    generate_c_files(
        path2save=path2save,
        template_name=params["template_name"],
        file_name="thresholding_mavg_abs",
        module_id=module_id,
...
def _generate_thresholding_mavg_abs_template() -> dict[str, list[str]]:
    header_template = [
        ...
        "DEF_NEW_MAVG_ABS_WINDOW_PROTO({$device_id}, {$data_type})",
    ]
    implementation_template = [
        ...
        '#include "{$path2include}/{$template_name}"',
        "DEF_CALC_MAVG_ABS_THR({$device_id}, {$data_type})",
        "DEF_NEW_MAVG_ABS_WINDOW_IMPL({$device_id}, {$data_type}, {$size}, {$gain_val})",
    ]
