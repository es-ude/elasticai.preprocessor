...
def build_thresholding_mavg_pow2(
    gain: float,
    window_size: int,
    bitwidth: int,
    signed: bool,
    ...
    """Generate C files for power-of-2 moving average thresholding.
    Args:
    ...
    params = {
        ...
        "template_name": "thresholding_mavg_pow2_template.h",
        "module_id": module_id,
        "device_id": module_id.upper(),
        "data_type": get_embedded_datatype(bitwidth, signed),
    }
    template_c = _generate_thresholding_mavg_pow2_template()
    generate_c_files(
        path2save=path2save,
        template_name=params["template_name"],
        file_name="thresholding_mavg_pow2",
        module_id=module_id,
...
def _generate_thresholding_mavg_pow2_template() -> dict[str, list[str]]:
    header_template = [
        "// --- Generating thresholding_mavg_pow2",
        "// Copyright @ UDE-IES",
        "// Code generated on: {$datetime_created}",
        "// Params: ID = {$device_id}, type = {$data_type}",
        '#include "{$path2include}/{$template_name}"',
        "DEF_NEW_MAVG_POW2_WINDOW_PROTO({$device_id}, {$data_type})",
    ]
    implementation_template = [
        ...
        '#include "{$path2include}/{$template_name}"',
        "DEF_CALC_MAVG_POW2_THR({$device_id}, {$data_type})",
        "DEF_NEW_MAVG_POW2_WINDOW_IMPL({$device_id}, {$data_type}, {$size}, {$gain_val})",
    ]
