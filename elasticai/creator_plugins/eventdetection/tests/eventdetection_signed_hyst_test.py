...
def test_template(
    ...
    with temporary_directory(backup):
        ...
        cocotb_test_fixture.set_top_module_name("EVENTDETECTOR_HYSTERESE_SIGNED")
        cocotb_test_fixture.clear_srcs()
        cocotb_test_fixture.add_srcs_from_package(
            "eventdetection",
            "verilog/eventdetector_hyst_signed.v",
        )
...
def test_build(
    ...
    with temporary_directory(backup) as tmpdir:
        ...
        cocotb_test_fixture.set_top_module_name("EVENTDETECTOR_HYST_SIGNED")
        cocotb_test_fixture.clear_srcs()
...
def test_equal(
    ...
    for vtype in [TargetsEventDetection.PosHyst]:
        ...
        with temporary_directory(backup) as tmpdir:
            ...
            cocotb_test_fixture.set_top_module_name(f"EVENTDETECTOR_HYST_SIGNED_{vtype.value.upper()}")
            cocotb_test_fixture.clear_srcs()
            cocotb_test_fixture.add_srcs_from_dir(path=tmpdir, glob_pattern="verilog/*.v")
            cocotb_test_fixture.run(
                params={},
