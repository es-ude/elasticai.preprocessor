...
@type_handler_iterable()
def replayer(impl: DataGraph, _: Registry) -> Iterable[Code]:
    package_path = "elasticai.creator_plugins.player"
    path2file = "verilog/replayer.v"

    _template = (
        TemplateDirector()
        .define_scoped_switch("ADD_TRIGGER", False)
        .parameter("BITWIDTH")
