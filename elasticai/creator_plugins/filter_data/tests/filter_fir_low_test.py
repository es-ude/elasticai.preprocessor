...
async def fir_simple_low(dut, bitwidth: int, fracwidth: int, data: list[int], check: list[int]):
    ...
    error = [a in [b - 1, b, b + 1] for a, b in zip(data_out, check)]
    passed = all(error)
    if not passed:
        print("Input:", data)
...
def test_template(
    ...
    data_in = [-16, -12, -8, -4, 0, 4, 8, 12, 13, 14, 15]
    data_check = [-8, -14, -10, -6, -2, 2, 6, 10, 12, 13, 14]
    ...
    with temporary_directory(backup):
        ...
        cocotb_test_fixture.add_srcs_from_package("filter_data", "verilog/filter_fir_low.v")
...
@pytest.mark.simulation
@pytest.mark.parametrize("bitwidth, fracwidth", [(6, 4)])
def test_build(
    cocotb_test_fixture: CocotbTestFixture,
    bitwidth: int,
    fracwidth: int,
