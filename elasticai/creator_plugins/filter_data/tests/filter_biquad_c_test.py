...
@pytest.fixture()
def tmp_path() -> Path:
    path2folder = get_path_to_project("build_test") / "fir_biquad"
...
def test_build(tmp_path: Path, target: str) -> None:
    ...
    with temporary_directory(backup) as tmpdir:
        ...
        assert (tmpdir / "filter_iir_low_0.c").exists()
        assert (tmpdir / "filter_iir_low_0.h").exists()
        assert (tmpdir / "filter_iir_template.h").exists()
