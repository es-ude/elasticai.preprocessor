...
@pytest.fixture()
def tmp_path() -> Path:
    path2folder = get_path_to_project("build_test") / "fir_half"
...
def test_build(tmp_path: Path, target: str) -> None:
    ...
    with temporary_directory(backup) as tmpdir:
        settings = SettingsFilter(1.0, 1000.0, 51, [100.0], "fir", "butter", "lowpass")
        Filtering(settings).create_design(target, bitwidth=8, id="0", path2save=tmpdir)
        assert (tmpdir / "filter_fir_low_0.c").exists()
        assert (tmpdir / "filter_fir_low_0.h").exists()
        assert (tmpdir / "filter_fir_template.h").exists()
