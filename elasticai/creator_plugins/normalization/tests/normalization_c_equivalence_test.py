...
def test_create_design_generates_minmax_c_files(tmp_path: Path, target: str) -> None:
    sets = SettingsNormalization(method="minmax", peak_mode=2)
...
def test_create_design_generates_zscore_c_files(tmp_path: Path, target: str) -> None:
    sets = SettingsNormalization(method="zscore", peak_mode=2)
    normalizer = DataNormalization(settings=sets)
    normalizer.create_design(target, 8, "0", tmp_path, signed=True)

    assert (tmp_path / "normalization_zscore_0.c").exists()
    assert (tmp_path / "normalization_zscore_0.h").exists()
    assert (tmp_path / "normalization_zscore_template.h").exists()
...
def test_create_design_rejects_other_normalization_methods(tmp_path: Path) -> None:
    sets = SettingsNormalization(method="norm", peak_mode=2)
    normalizer = DataNormalization(settings=sets)
...
def test_create_design_rejects_other_peak_modes(tmp_path: Path) -> None:
    sets = SettingsNormalization(method="minmax", peak_mode=1)
    normalizer = DataNormalization(settings=sets)
...
def test_generated_zscore_c_matches_python_frames(
    ...
    adapter = tmp_path / "adapter.h"
    adapter.write_text(
        f"void normalize_zscore_0(const {c_type} *input, float *output, unsigned int length);\n"
    )
    loader = CompileLoader(
        headers=str(adapter),
        sources=[str(output_dir / "normalization_zscore_0.c")],
        build_dir=str(tmp_path / "cffi-build"),
        module_name=f"normalization_zscore_equivalence_{uuid4().hex}",
    )
    loader.load()
    ...
    normalize_c = loader.get("normalize_zscore_0")
...
def test_generated_zscore_c_accepts_empty_frame(
    ...
    loader = CompileLoader(
        ...
        sources=[str(output_dir / "normalization_zscore_0.c")],
        build_dir=str(tmp_path / "cffi-build"),
        module_name=f"normalization_zscore_empty_{uuid4().hex}",
    )
    loader.load()
    c_input = loader.ffi().new(f"{c_type}[]", [0])
    c_output = loader.ffi().new("float[]", [23.0])

    loader.get("normalize_zscore_0")(c_input, c_output, 0)
