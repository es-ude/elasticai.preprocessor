...
class TestTransformation(TestCase):
    ...
    def test_fft_value(self):
        ...
        chck_amp = all(
            [
                is_close(spec[pred_frq], true_amp, 0.1)
                for pred_frq, true_amp in zip(x[0], [1.0, 0.25])
            ]
        )
        chck_frq = all(
            [
                is_close(freq[pred_frq], true_frq, 0.1)
                for pred_frq, true_frq in zip(x[0], [100.0, 1000.0])
            ]
        )
