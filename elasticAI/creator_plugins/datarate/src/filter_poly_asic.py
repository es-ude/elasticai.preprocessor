...
def filter_poly_asic(impl: DataGraph) -> Iterable[Code]:
    ...
    _template = (
        ...
        .add_module_name()
        .set_prototype(res.read_text(package_path, f"verilog/filter_polydec_asic.v"))
        .build()
    )
    ...
    if impl.data['build_tb']:
        _testbench = (
            ...
            .replace_instance_name("FILTER_POLYDEC_ASIC", impl.name.upper())
            .add_module_name()
            .set_prototype(res.read_text(package_path, f"verilog/filter_polydec_asic_tb.v"))
            .build()
        )
